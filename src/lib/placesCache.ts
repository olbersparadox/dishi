...
/** Round to ~111m grid cells so nearby lookups share a cache entry. */
function bucketFor(lat: number, lng: number): string {
 ...
 */
export async function cachedNearbyPlaces(lat: number, lng: number): Promise<GooglePlace[]> {
  const admin = supabaseAdmin();
  const bucket = bucketFor(lat, lng);
  ...
  const results = await searchNearbyRestaurants(lat, lng);
