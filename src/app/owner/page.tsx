...
type OwnerMenuItem = {
  id: string; name: string; name_zh: string | null; name_original: string | null;
  description: string | null; price: string | null; cuisine: string | null; available: boolean;
};
type OwnerOrder = {
    ...
    return (
      <div>
        ...
        <p className="scan-banner">
          Unverified claim &mdash; analytics are live, but public &ldquo;owner&rdquo; features stay off until real verification exists.
        </p>

        <div className="owner-tabs" role="tablist">
          {(['overview', 'menu', 'tables', 'orders'] as const).map(tb => (
            <button key={tb} role="tab" aria-selected={tab === tb}
              className={tab === tb ? 'on' : ''} onClick={() => setTab(tb)}>
              {tb === 'overview' ? 'Overview' : tb === 'menu' ? 'Menu' : tb === 'tables' ? 'Tables & QR' : 'Orders'}
            </button>
          ))}
        </div>
        ...
        {dash.loved_for.length > 0 && (
          <div className="card"><div className="card-body">
            ...
            <p className="card-meta" style={{ marginTop: 8 }}>
              From the flavor profiles of your positively-rated dishes &mdash; the kitchen&rsquo;s strengths as diners actually taste them.
            </p>
          </div></div>
        )}
        ...
        {dash.hidden_gems.length > 0 && (
          <div className="card scan-hero"><div className="card-body">
            ...
            <p className="card-meta" style={{ marginBottom: 10 }}>
              Rated well above your average, but rarely logged &mdash; candidates for the specials board.
            </p>
  ...
  return (
    <div>
      ...
      <p className="card-meta" style={{ marginBottom: 16 }}>
        Claim your restaurant to see how diners&rsquo; palates respond to your menu.
      </p>
      {error && <p style={{ color: 'var(--lacquer)' }}>{error}</p>}
      {restaurants === null && <p className="card-meta">Loading restaurants&hellip;</p>}
...
function MenuTab({ restaurantId }: { restaurantId: string }) {
  ...
  async function importMenu(file: File | null) {
    ...
    const json = await res.json();
    setMsg(res.ok
      ? `Imported ${json.imported} dishes from the photo${json.mock ? ' (demo data \u2014 no OPENROUTER_API_KEY set)' : ''}.`
  ...
  return (
    <div>
      ...
      {items === null && <p className="card-meta">Loading the menu&hellip;</p>}
      {items?.length === 0 && <p className="card-meta">No dishes yet &mdash; import or add above.</p>}
      {items?.map(i => (
        <div className="card" key={i.id}><div className="card-body" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
          <div style={{ minWidth: 0, opacity: i.available ? 1 : 0.45 }}>
            <div className="card-title" style={{ fontSize: 15 }}>
              <DishName name={i.name} name_zh={i.name_zh} name_original={i.name_original} />
            </div>
            <div className="card-meta">
              {i.price ?? 'no price'}{i.description ? ` \u00b7 ${i.description}` : ''}
            </div>
          </div>
...
function TablesTab({ restaurantId }: { restaurantId: string }) {
  ...
  async function regenerate(tbl: OwnerTable) {
    if (!confirm(`Regenerate the QR for ${tbl.label}? Every printed copy of the old code stops working.`)) return;
    const res = await fetch('/api/restaurant/tables', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ table_id: tbl.id }),
    });
    if (res.ok) { await load(); setShowQr(tbl.id); }
  }

  async function remove(tbl: OwnerTable) {
    if (!confirm(`Remove ${tbl.label}? Its QR code stops working.`)) return;
  ...
  return (
    <div>
      <div className="card"><div className="card-body">
        ...
        <p className="card-meta" style={{ marginTop: 8 }}>
          Each table gets its own QR to print and laminate. Diners scan it to see the menu ranked for their taste &mdash; and order.
        </p>
      </div></div>
      {msg && <p className="scan-banner">{msg}</p>}

      {tables === null && <p className="card-meta">Loading tables&hellip;</p>}
      {tables?.length === 0 && <p className="card-meta">No tables yet.</p>}
      {tables?.map(tbl => (
        <div className="card" key={tbl.id}><div className="card-body">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <div className="card-title" style={{ fontSize: 15.5 }}>{tbl.label}</div>
            <div style={{ display: 'flex', gap: 6 }}>
              <button className="btn small" onClick={() => setShowQr(showQr === tbl.id ? null : tbl.id)}>
                {showQr === tbl.id ? 'Hide QR' : 'Show QR'}
              </button>
              <button className="btn ghost small" onClick={() => regenerate(tbl)}>Regenerate</button>
              <button className="btn ghost small" onClick={() => remove(tbl)}>Remove</button>
            </div>
          </div>
          {showQr === tbl.id && <div style={{ marginTop: 12 }}><TableQR token={tbl.qr_token} label={tbl.label} /></div>}
        </div></div>
      ))}
    </div>
  );
...
function OrdersTab({ restaurantId }: { restaurantId: string }) {
  ...
  if (orders === null) return <p className="card-meta">Loading the queue&hellip;</p>;
      ...
      {orders.map(o => (
        <div className={`card order-ticket ${o.status}`} key={o.id}><div className="card-body">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            ...
            <span className="card-meta">{new Date(o.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} &middot; {o.diner}</span>
          </div>
          <div style={{ margin: '6px 0 10px' }}>
            {o.items.map((i, idx) => (
              <div key={idx} style={{ fontSize: 14.5 }}>
                <strong>{i.qty}&times;</strong> {i.name} {i.price ? <span className="card-meta">&middot; {i.price}</span> : null}
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
