...
## G4 round 2 tune 5: 鴨鵝 thickness stacked another +20% — *(Sonnet)* — ✅ SHIPPED 2026-08-07
Owner, on the wing bench, immediately after the goose dial above: "duck /
goose stroke thickness increase another 20%." Same "another" reading as the
earlier 雞 stack — a second pass on top of the existing +20%, not a
replacement. `gooseThicknessMul` became `(1 + 0.2 * gooseBoost) * (1 + 0.2 *
gooseBoost)`, mirroring the 雞 stack's shape exactly. At pure 鴨鵝 the plain
blend's widthMul 0.7 now carries a 1.2 × 1.2 = 1.44× stack, landing at 1.008
— curvature stays untouched (no dial was ever requested for this side).

Verifying it surfaced a real (and, on inspection, harmless) wrinkle: unlike
雞's blend and boost which both move in the SAME direction with k, 鴨鵝's
plain-blend widthMul TRENDS DOWN as lean grows (0.7 at pure) while its
thickness stack trends UP — once the stack's near-neutral slope (~0.4, from
compounding two +20% passes) exceeds the blend's own 0.3 coefficient, the
product very slightly overshoots before settling: it peaks near 1.029
around a 55-65% 鴨鵝 lean before landing at 1.008 at the pure endpoint,
instead of decreasing monotonically the whole way like every other dial
shipped so far. Confirmed on paper (a quick numeric trace across the full
range) and then confirmed harmless: the overshoot is ~2% of a stroke width
that's already scaled by `R * 0.05`, sub-pixel at any realistic canvas size,
and invisible on `/dev-wings`'s screenshots. The existing "ramps
continuously, never jumps" test for this dial was rewritten to assert what's
actually true — neutral=1, pure=full stack, overshoot bounded under 1.03 —
rather than a strict monotonic-everywhere claim that this dial's own math no
longer satisfies. Documented in the test itself so the next reader isn't
surprised.

Verified: 11-fixture byte-identity sweep — legacy identical across all 11,
metabolism identical for the 9 fixtures with no goose-leaning air data,
only `goose` and `gooseLean` changed again. Visual check on `/dev-wings`:
鴨鵝·pure and 鴨鵝-lean both read distinctly heavier than the prior round,
now roughly matching 雞's presence; 雞 and generic cells unchanged; legacy
row stays flat.

3 test assertions updated, 1 rewritten with the overshoot documented. tsc
clean; full suite 1388/1388.
