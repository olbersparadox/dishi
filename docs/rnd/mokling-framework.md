...
| 鰭 fins | base pair (尖 · 圓 · 帶 lab-only) | **SHIPPED (base form)** 2026-08-08 (G4 round 4, `finPlan`) — TRACES 其三's approved pair (one pointed leaf per flank, behind the body), the fish pool's SECOND part: tail claims at bud, fins bud at saturation (12), independent of the tail slot's cross-family contest. Flutter shares 魚尾's swim clock (same constants, same phase). Metabolism only; legacy pinned null. The three METHOD variants stay lab-only: 帶 needs a 長身魚 detector ("no detector, no feature"), 尖/圓 await their own rounds on top of the approved base | 尖/圓 method conditioning; 帶 needs 長身魚 detector |
