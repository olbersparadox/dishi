...
| 尾 tails | 魚 · 甲殼 · 牛 · 豬 · 禽 | **SHIPPED** 2026-08-07 (G4 round 3, `tailPlan`/`drawTail`) — ONE tail slot, Decision 6 vacancy → priority: 魚 forks as the fish sub-node's first part (fins unported), 牛/豬 claim first when the OTHER species holds the foot, 甲殼/禽 bud as second parts at saturation depth (12). Metabolism only; legacy pinned null. 魚尾 follows the owner's APPROVED trace (v7 TRACES 其二 — stemless two-fluke off the rim), not VOCAB's peduncle specimen; 牛/豬 still use VOCAB's stroke forms, and TRACES 其六's "THICK … not a wire" correction has NOT been applied to them | — (the old "魚 and 禽 need their sub-nodes" was stale — G3 shipped both detectors) |
...
| 鰭 fins | base pair (尖 · 圓 · 帶 lab-only) | **SHIPPED (base form)** 2026-08-08 (G4 round 4, `finPlan`) — the fish pool's SECOND part: tail claims at bud, fins bud at saturation (12), independent of the tail slot's cross-family contest. Anchor/gate/motion still trace to TRACES 其三 (base at 0.92 of centre→flank, seat 1.48 rad, bisector 0.29 rad above horizontal, flutter sharing 魚尾's swim clock); DRAWING TECHNIQUE switched same-day from a single filled leaf to 翼's own per-stroke fan (owner: "the treatment of wings should fit for fin, fan shaped with possible more thin strokes") — 5 tapering strokes per flank, same falloff/hump convention as `S.wings.on`'s loop. SIZE range is fin-specific, not shared with TAIL_MIN (owner, same day: "treat this size of freshly budded. Then increase by 200% for full grown size") — `sizeF` now runs 1.0 (bud) → 3.0 (full), a literal 3× increase over the bud reference. Metabolism only; legacy pinned null. The three METHOD variants stay lab-only: 帶 needs a 長身魚 detector ("no detector, no feature"), 尖/圓 await their own rounds on top of the approved base | 尖/圓 method conditioning; 帶 needs 長身魚 detector |
